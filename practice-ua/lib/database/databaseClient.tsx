...
export class DatabaseClient {
  ...
  async init() {
    ...
    await this.db.execAsync(
      `CREATE TABLE IF NOT EXISTS ${dayStatTableName}(date TEXT, lesson_name TEXT, num_correct INTEGER, num_answered INTEGER, PRIMARY KEY(date, lesson_name))`
    );
    console.log(`[DatabaseClient] Ensured table exists: ${dayStatTableName}`);

    // Create the starred lessons table
    await this.db.execAsync(
      `CREATE TABLE IF NOT EXISTS ${starredLessonsTableName}(lesson_name TEXT, PRIMARY KEY(lesson_name))`
    );
    console.log(
      `[DatabaseClient] Ensured table exists: ${starredLessonsTableName}`
    );
